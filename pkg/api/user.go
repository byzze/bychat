...
func LogOut(appID, userID uint32) {
	...
	websocket.UnregisterChannel(client)
}
...
func GetRoomUserList(appID, roomID uint32) (userList []*models.ResponseUserOnline) {
	...
	for i, server := range servers {
		if server.IP == websocket.GetServerNode().IP && server.Port == websocket.GetServerNode().Port {
...
func UserSendMessageAll(appID, roomID, userID uint32, message string) (sendResults bool, err error) {
	...
	websocket.SendMsgAllServer(appID, roomID, userID, message)
	return
}
