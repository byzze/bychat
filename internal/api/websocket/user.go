...
// BindUser 绑定用户信息
func BindUser(client *models.Client, seq string, message []byte) (code uint32, msg string, data interface{}) {
	code = common.OK
	...
	ws.BinUserdChannel(client)
	return
}

// Heartbeat 心跳
func Heartbeat(client *models.Client, seq string, message []byte) (code uint32, msg string, data interface{}) {
	code = common.OK
	...
	if !ws.GetClientManager().IsLogin(client) {
		logrus.WithFields(logrus.Fields{
			"UserId": request.UserID,
			"seq":    seq,
